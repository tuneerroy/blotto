...
def play(p1, p2, iterations=10000, UNITS=100):
    ...
    p1_choices = np.random.choice(n, (iterations, UNITS), replace=True) # (iterations, UNITS)
    p2_choices = np.random.choice(n, (iterations, UNITS), replace=True) # (iterations, UNITS)
    p1_alloc = np.apply_along_axis(lambda x : np.histogram(x, bins=np.arange(n+1))[0], axis=1, arr=p1_choices) # (iterations, n)
    p2_alloc = np.apply_along_axis(lambda x : np.histogram(x, bins=np.arange(n+1))[0], axis=1, arr=p2_choices) # (iterations, n)
...
def mw(experts: list[Expert], oponent: Expert, eta=0.1, T=1000):
    n = len(experts)
    weights = np.ones(n)
    total_loss = 0
    for _ in range(T):
