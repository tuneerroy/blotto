...
def get_allocations(p, iterations):
    ...
    p_choices = np.random.choice(
        len(p), (iterations, UNITS), replace=True, p=p
    )  # (iterations, UNITS)
    p_alloc = np.apply_along_axis(
        lambda x: np.histogram(x, bins=np.arange(len(p) + 1))[0], axis=1, arr=p_choices
    )  # (iterations, n)
    return p_alloc
...
def experts_play(experts, opponent, iterations=1000):
    ...
    return np.apply_along_axis(
        lambda x: play(get_allocations(x, iterations), opponent_alloc),
        axis=1,
        arr=experts,
    )
...
def mw(experts: list, opponent, eta=0.1, T=100):
    ...
    for _ in range(T):
        ...
        losses_expected = results_expected[:, 1] / (
            results_expected[:, 0] + results_expected[:, 1]
        )
